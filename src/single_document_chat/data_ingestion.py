    ...
    def ingest_files(self, uploaded_files):
        try:
            ...
            for uploaded_file in uploaded_files:
                unique_filename  =  f"session_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:8]}.pdf"
                ...
                self.log.info("PDF loaded successfully", count=len(documents))
